...
class IkuaiClient:
    ...
    def __init__(self, url: str, username: str, password: str, plugin_name: str = "iKuai"):
        self.url = url
    ...
    def login(self) -> bool:
        ...
        try:
            ...
            logger.error(f"{self.plugin_name} 登录成功但未能提取 sess_key。响应: {response.text[:200]}")
            return False
    ...
    def create_backup(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            try:
                ...
                err_msg = res_json.get("errmsg") or res_json.get("ErrMsg", "创建备份API未返回成功")
                logger.error(f"{self.plugin_name} 备份创建失败 (JSON)。响应: {res_json}, 错误: {err_msg}")
                return False, f"路由器返回错误: {err_msg}"

            except json.JSONDecodeError:
                logger.error(f"{self.plugin_name} 备份创建失败，非JSON响应。响应: {response_text}")
    ...
    def get_backup_list(self) -> Optional[List[Dict]]:
        ...
        list_data_old = {"func_name": "backup", "action": "show", "param": {}}
        request_headers = {
            'Content-Type': 'application/json',
            'Accept': '*/*',
            'Origin': self.url.rstrip('/'),
        ...
        try:
            ...
            if res_json.get("code") == 0 and str(res_json.get("message", "")).lower() in ["success", "ok", "成功"]:
                ...
                if isinstance(backup_info, list) and backup_info:
                    ...
                    logger.warning(f"{self.plugin_name} backup_info 为空或格式不正确。")
                    return []
            if (
                (res_json.get("Result") == 30006 and "unknown TYPE" in res_json.get("ErrMsg", "")) or
                ("unknown TYPE" in str(res_json))
            ):
                logger.warning(f"{self.plugin_name} 检测到老版本API，尝试兼容老版本参数...")
                ...
                if res_json_old.get("Result") == 30000 and res_json_old.get("ErrMsg", "").lower() == "success":
                    ...
                    err_msg = res_json_old.get("ErrMsg") or res_json_old.get("message") or "获取列表失败"
                    logger.error(f"{self.plugin_name} (老版本) 获取备份列表失败: {err_msg}")
                    return None
            err_msg = res_json.get("message") or res_json.get("ErrMsg") or "获取列表失败"
            logger.error(f"{self.plugin_name} 获取备份列表失败: {err_msg}")
            return None
        except requests.exceptions.RequestException as e:
            logger.error(f"{self.plugin_name} 获取备份列表请求失败: {e}")
            return None
        except json.JSONDecodeError:
            logger.error(f"{self.plugin_name} 获取备份列表响应非JSON格式")
            return None
        except Exception as e:
    ...
    def download_backup(self, router_filename: str, local_filepath: str) -> Tuple[bool, Optional[str]]:
        ...
        logger.info(f"{self.plugin_name} 尝试下载备份文件 {router_filename}")
        try:
            with self.session.get(download_url, stream=True, timeout=300, headers=request_headers) as r:
                ...
                with open(local_filepath, 'wb') as f:
                    for chunk in r.iter_content(chunk_size=8192):
                        f.write(chunk)
                logger.info(f"{self.plugin_name} 文件 {router_filename} 下载完成")
                return True, None
    ...
    def restore_backup(self, filename: str, backup_content: bytes) -> Tuple[bool, Optional[str]]:
        ...
        restore_payload = {"func_name": "backup", "action": "RESTORE", "param": {}}
        try:
            ...
            files = {'file': (filename, backup_content, 'application/octet-stream')}
            ...
            try:
                ...
                if result.get("Result") == 30000 or (isinstance(result, str) and "success" in result.lower()):
                    ...
                    return False, result.get("ErrMsg") or result.get("errmsg", "恢复失败")
            except json.JSONDecodeError:
                if "success" in upload_response.text.lower():
                    return True, None
                return False, f"恢复失败: {upload_response.text[:200]}"
        except requests.exceptions.RequestException as e:
    ...
    def delete_backup(self, filename: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            logger.info(f"{self.plugin_name} 尝试删除备份文件: {filename}...")
            response = self.session.post(delete_url, data=json.dumps(delete_data), headers=request_headers, timeout=30)
            response.raise_for_status()
            try:
                res_json = response.json()
                if res_json.get("Result") == 30000 and "success" in res_json.get("ErrMsg", "").lower():
                    return True, None
                return False, res_json.get("ErrMsg", "删除失败")
            except json.JSONDecodeError:
    ...
    def get_system_info(self) -> Optional[Dict]:
        ...
        try:
            ...
            if data is not None:
                ...
                if cpu_list:
                    cpu_value = float(cpu_list[-1].replace("%", ""))
                    system_info["cpu_usage"] = cpu_value
                memory = data.get("memory", {})
                if memory:
                    system_info["mem_usage"] = float(memory.get("used", "0%").replace("%", ""))
            ...
            logger.error(f"{self.plugin_name} 获取系统信息失败: {e}")
            return None
    ...
    def get_monitor_info(self) -> Optional[Dict]:
        ...
        try:
            response = self.session.post(monitor_url, data=json.dumps(monitor_data), headers=request_headers, timeout=10)
            response.raise_for_status()
            res_json = response.json()
            if res_json.get("Result") == 30000 and res_json.get("ErrMsg", "").lower() == "success":
            ...
            logger.error(f"{self.plugin_name} 获取监控信息失败: {e}")
            return None
    ...
    def get_interface_info(self) -> Optional[Dict]:
        ...
        try:
            ...
            if lan_json.get("code") == 0 and str(lan_json.get("message", "")).lower() in ["success", "ok", "成功"]:
                snapshoot_lan = lan_json.get("results", {}).get("snapshoot_lan")
            elif lan_json.get("Result") == 30000 and lan_json.get("ErrMsg", "").lower() == "success":
                snapshoot_lan = lan_json.get("Data", {}).get("snapshoot_lan")
            ...
            logger.error(f"{self.plugin_name} 获取接口信息失败: {e}")
            return None
